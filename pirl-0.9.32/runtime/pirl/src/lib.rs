...
use pirl_runtime_constants::{currency::*, fee::*, time::*};
...
impl_runtime_weights!(pirl_runtime_constants);
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	...
	impl_name: create_runtime_str!("pgrocks-pirl"),
	authoring_version: 0,
	spec_version: 9320,
	impl_version: 0,
...
impl frame_support::traits::OnRuntimeUpgrade for InitiateNominationPools {
	fn on_runtime_upgrade() -> frame_support::weights::Weight {
		...
		if pallet_nomination_pools::MaxPools::<Runtime>::get().is_none() {
			...
			log::info!(target: "runtime::pirl", "pools config initiated 🎉");
			<Runtime as frame_system::Config>::DbWeight::get().reads_writes(1, 5)
		} else {
			log::info!(target: "runtime::pirl", "pools config already initiated 😏");
...
sp_api::impl_runtime_apis! {
	...
	impl frame_try_runtime::TryRuntime<Block> for Runtime {
		...
		fn execute_block(block: Block, state_root_check: bool, select: frame_try_runtime::TryStateSelect) -> Weight {
			log::info!(
				target: "runtime::pirl", "try-runtime: executing block #{} ({:?}) / root checks: {:?} / sanity-checks: {:?}",
				block.header.number,
				block.header.hash(),
				state_root_check,
				select,
